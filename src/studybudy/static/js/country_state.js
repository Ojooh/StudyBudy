jQuery(document).ready(function( $ ) {
    ...
    //     $(`#state option[value='${state_val}']`).prop('selected', true);
    // }
    ...
    state.change(function setValue(){
      var cix = $(this).val();
      $("#state-val").val(cix)

    });

    if (($("#country").val() == "" || $("#state").val() == "") && ($("#country-val").val() != "" || $("#state-val").val() != "")){
      
        var db_ctry = $("#country-val").val();
        var db_state  = $("#state-val").val();
        // console.log(db_ctry);
        
        for (var t = 0; t < countries.length; t++){
          var ctry = countries[t].country;
          if (typeof db_ctry != "undefined"){
            if (ctry.toLowerCase() == db_ctry.toLowerCase()){
              var ctry_val = ctry + "-" + t;
              break
            }
          }else{
        ...
        $("#country").val(ctry_val);
        var valls = ctry_val.split("-");
        var states = countries[valls[1]].states;
        state.empty();
        html_state = "<option value=''><!-----choose----></option>";
        for (var u = 0; u < states.length; u++){
